				...
				bool chg_online)
{
	...
	pr_no_info("charger type: %s: online = %d, type = %d\n", __func__,
		chg_online, chg_type);
	...
	if (chg_online) {
		...
		if (ret < 0)
			pr_no_notice("%s: psy online failed, ret = %d\n",
				__func__, ret);
		...
		if (ret < 0)
			pr_no_notice("%s: psy type failed, ret = %d\n",
				__func__, ret);
...
static void hw_bc11_init(void)
{
	...
	if (first_connect == true) {
		...
		if (is_usb_rdy() == false) {
			pr_no_info("CDP, block\n");
			while (is_usb_rdy() == false && timeout > 0) {
				msleep(100);
				timeout--;
			}
			if (timeout == 0)
				pr_no_info("CDP, timeout\n");
			else
				pr_no_info("CDP, free\n");
		} else
			pr_no_info("CDP, PASS\n");
...
static unsigned int hw_bc11_stepB2(void)
{
	...
	if (wChargerAvail == 1) {
		bc11_set_register_value(PMIC_RG_BC11_VSRC_EN, 0x2);
		pr_no_info("charger type: DCP, keep DM voltage source in stepB2\n");
	}
	return wChargerAvail;
...
static void dump_charger_name(enum charger_type type)
{
	...
	case CHARGER_UNKNOWN:
		pr_no_info("charger type: %d, CHARGER_UNKNOWN\n", type);
		break;
	case STANDARD_HOST:
		pr_no_info("charger type: %d, Standard USB Host\n", type);
		break;
	case CHARGING_HOST:
		pr_no_info("charger type: %d, Charging USB Host\n", type);
		break;
	case NONSTANDARD_CHARGER:
		pr_no_info("charger type: %d, Non-standard Charger\n", type);
		break;
	case STANDARD_CHARGER:
		pr_no_info("charger type: %d, Standard Charger\n", type);
		break;
	case APPLE_2_1A_CHARGER:
		pr_no_info("charger type: %d, APPLE_2_1A_CHARGER\n", type);
		break;
	case APPLE_1_0A_CHARGER:
		pr_no_info("charger type: %d, APPLE_1_0A_CHARGER\n", type);
		break;
	case APPLE_0_5A_CHARGER:
		pr_no_info("charger type: %d, APPLE_0_5A_CHARGER\n", type);
		break;
	default:
		pr_no_info("charger type: %d, Not Defined!!!\n", type);
		break;
	}
}
...
int hw_charging_get_charger_type(void)
{
...
#ifdef CONFIG_MTK_USB2JTAG_SUPPORT
	if (usb2jtag_mode()) {
		pr_no_info("[USB2JTAG] in usb2jtag mode, skip charger detection\n");
	...
	if (CHR_Type_num != STANDARD_CHARGER)
		...
		pr_no_info("charger type: skip bc11 release for BC12 DCP SPEC\n");
	...
	CHR_Type_num = STANDARD_HOST;
	pr_no_info("charger type: Froce to STANDARD_HOST\n");
#endif
...
void nostandard_detect_retry(void)
{
	...
	if (g_chr_type == NONSTANDARD_CHARGER) {
		for (loop = 0; loop < 2; loop ++) {
			...
			pr_no_info("charger type: NONSTANDARD,LOOP = %d\n", loop);
			if (g_chr_type != NONSTANDARD_CHARGER)
				break;
		}
	}
	pr_no_info("[nostand_detect_retry] charger type = %d\n", g_chr_type);
}
...
void mtk_pmic_enable_chr_type_det(bool en)
{
	...
	if (!mt_usb_is_device()) {
		...
		pr_no_info("charger type: UNKNOWN, Now is usb host mode. Skip detection\n");
		return;
	}
#endif
	...
	if (en) {
		if (is_meta_mode()) {
			...
			pr_no_notice("charger type: force Standard USB Host in meta\n");
			g_chr_type = STANDARD_HOST;
			chrdet_inform_psy_changed(g_chr_type, 1);
		} else {
			pr_no_info("charger type: charger IN\n");
			g_chr_type = hw_charging_get_charger_type();
			nostandard_detect_retry();
			chrdet_inform_psy_changed(g_chr_type, 1);
		}
	} else {
		pr_no_info("charger type: charger OUT\n");
		g_chr_type = CHARGER_UNKNOWN;
...
{
	/*
	 * pr_no_notice("[chrdet_int_handler]CHRDET status = %d....\n",
	...
	if (!pmic_get_register_value(PMIC_RGS_CHRDET)) {
		...
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
			...
			pr_no_info("[chrdet_int_handler] Unplug Charger/USB\n");
...
static int __init pmic_chrdet_init(void)
{
	...
	if (!chrdet_psy) {
		pr_no_notice("%s: get power supply failed\n", __func__);
		return -EINVAL;
	}
