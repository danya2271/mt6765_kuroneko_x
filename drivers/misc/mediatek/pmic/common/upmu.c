				     ...
				     unsigned int SHIFT)
{
	...
	pmic_reg = rdata;
	if (return_value != 0) {
		pr_no_info("[%s] err ret=%d, Reg[0x%x] pmic_wrap read data fail, MASK=0x%x, SHIFT=%d\n"
				      ...
				      unsigned int SHIFT)
{
	...
	pmic_reg = rdata;
	if (return_value != 0) {
		pr_no_info("[%s] err ret=%d, Reg[0x%x] pmic_wrap read data fail, val=0x%x, MASK=0x%x, SHIFT=%d\n"
	...
	if (return_value != 0) {
		pr_no_info("[%s] err ret=%d, Reg[0x%x]=0x%x pmic_wrap write data fail, val=0x%x, MASK=0x%x, SHIFT=%d\n"
	...
	if (return_value)
		pr_no_info("[%s]IPIMB write data fail with ret = %d, (0x%x,0x%x,0x%x,%d)\n"
			, __func__, return_value, RegNum, val, MASK, SHIFT);
	else
				       ...
				       unsigned int val)
{
	...
	if (pFlag->flagname != flagname) {
		pr_no_info("[%s]pmic flag idx error\n", __func__);
		return 1;
	}
	...
	if (ret != 0) {
		pr_no_info("[%s] error ret: %d when set Reg[0x%x]=0x%x\n",
...
unsigned short pmic_get_register_value(PMU_FLAGS_LIST_ENUM flagname)
{
	...
	ret = pmic_read_interface((unsigned int)pFlag->offset, &val,
		(unsigned int)(pFlag->mask), (unsigned int)(pFlag->shift));
	if (ret != 0) {
		pr_no_info("[%s] error ret: %d when get Reg[0x%x]\n", __func__,
			ret, (pFlag->offset));
		return ret;
	}
				     ...
				     unsigned int val)
{
	...
	if (pFlag->flagname != flagname) {
		pr_no_notice("[%s]pmic flag idx error\n", __func__);
		return 1;
	}
	...
	if (ret != 0) {
		pr_no_notice("[%s] error ret: %d when set Reg[0x%x]=0x%x\n"
		       , __func__
		       , ret
		       , (pFlag->offset)
		       , val);
...
{
	pr_no_notice("[%s] smart reset not support!\n", __func__);
}
...
static int __init pmic_mt_init(void)
{
	...
	if (ret) {
		pr_no_info("****[%s] Unable to register driver (%d)\n"
			, __func__, ret);
		return ret;
	}

	no_printk("****[%s] Initialization : DONE !!\n", __func__);
