 ...
 */
	...
	preempt_disable();
	local_irq_disable();
	pr_no_info(PMICTAG "[%s][pmic_boot_status]\n", __func__);
				 ...
				 size_t size)
{
	...
	pr_no_info("[%s]\n", __func__);
	if (buf != NULL && size != 0) {
		pr_no_info("[%s] size is %d, buf is %s\n"
		...
		if (val) {
			...
			pr_no_info("[%s] write PMU reg 0x%x with value 0x%x !\n"
			...
			pr_no_info("[%s] read PMU reg 0x%x with value 0x%x !\n"
				, __func__, reg_address, g_reg_value);
			pr_no_info("[%s] use \"cat pmic_access\" to get value\n"
				,  __func__);
		}
	}
	return size;
...
int pmic_debug_init(struct platform_device *dev)
{
	...
	if (!mtk_pmic_dir) {
		pr_no_notice(PMICTAG "fail to mkdir /sys/kernel/debug/mtk_pmic\n");
		return -ENOMEM;
	}
