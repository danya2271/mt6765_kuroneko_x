...
struct ccci_smem_region md1_6295_noncacheable_fat[] = {
...
struct ccci_smem_region md1_6295_cacheable[] = {
/*
...
void ccci_6295_md_smem_layout_config(struct ccci_modem *md)
{
	...
	/* non-cacheable start */
	for (i = 1; i < (sizeof(md1_6295_noncacheable_fat)/
		sizeof(struct ccci_smem_region)); i++) {
		update_smem_region(&md1_6295_noncacheable_fat[i]);
		if (md1_6295_noncacheable_fat[i].size == 0)
			/* update offset */
			md1_6295_noncacheable_fat[i].offset =
			md1_6295_noncacheable_fat[i-1].offset
			+ md1_6295_noncacheable_fat[i-1].size;

		/* Special case */
		switch (md1_6295_noncacheable_fat[i].id) {
	...
	mm_str->md_bank4_noncacheable = md1_6295_noncacheable_fat;
	...
	get_md_cache_region_info(SMEM_USER_CCB_START,
				&ccb_offset,
				&ccb_size);
	for (i = 0; i < (sizeof(md1_6295_cacheable)/
		sizeof(struct ccci_smem_region)); i++) {

		switch (md1_6295_cacheable[i].id) {
		case SMEM_USER_CCB_DHL:
		case SMEM_USER_CCB_MD_MONITOR:
		case SMEM_USER_CCB_META:
			md1_6295_cacheable[i].size =
				(ccb_size > CCB_CACHE_MIN_SIZE) ?
				CCB_CACHE_MIN_SIZE:0;
			md1_6295_cacheable[i].offset =  ccb_offset;
			break;
		...
		case SMEM_USER_RAW_USIP:
			get_md_cache_region_info(md1_6295_cacheable[i].id,
				&md_resv_mem_offset,
				&md_resv_mem_size);

			md1_6295_cacheable[i].size = md_resv_mem_size;
			if (md_resv_mem_offset || md_resv_mem_size)
				md1_6295_cacheable[i].offset =
					md_resv_mem_offset; /* LK config */
			else if (i == 0)
				md1_6295_cacheable[i].offset = 0;
			else
				md1_6295_cacheable[i].offset =
					md1_6295_cacheable[i - 1].offset +
					md1_6295_cacheable[i - 1].size;

			break;
		default:
			md1_6295_cacheable[i].size = 0;
			md1_6295_cacheable[i].offset = 0;
			break;
		}
	}

	mm_str->md_bank4_cacheable = md1_6295_cacheable;
}
...
#if (MD_GENERATION >= 6295)
	ap_side_md_feature->
		feature_set[MD_WIFI_PROXY_SHARE_MEMORY].support_mask =
		CCCI_FEATURE_OPTIONAL_SUPPORT;
#else
	ap_side_md_feature->
		feature_set[MD_WIFI_PROXY_SHARE_MEMORY].support_mask =
...
#if (MD_GENERATION >= 6295)
	ap_side_md_feature->
		feature_set[NVRAM_CACHE_SHARE_MEMORY].support_mask =
		CCCI_FEATURE_MUST_SUPPORT;
#else
	ap_side_md_feature->
		feature_set[NVRAM_CACHE_SHARE_MEMORY].support_mask =
