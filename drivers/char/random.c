...
static int crng_slow_load(const char *cp, size_t len)
{
	...
	unsigned char		tmp;
	unsigned		i, max = BLAKE2S_KEY_SIZE;
	...
	for (i = 0; i < max ; i++) {
		...
		if (tmp & 1)
			lfsr ^= 0xE1;
		tmp = dest_buf[i % BLAKE2S_KEY_SIZE];
		dest_buf[i % BLAKE2S_KEY_SIZE] ^= src_buf[i % len] ^ lfsr;
		lfsr += (tmp << 3) | (tmp >> 5);
	}
	spin_unlock_irqrestore(&primary_crng.lock, flags);
	return 1;
...
struct batched_entropy {
	union {
		u64 entropy_u64[BLAKE2S_BLOCK_SIZE / sizeof(u64)];
		u32 entropy_u32[BLAKE2S_BLOCK_SIZE / sizeof(u32)];
	};
