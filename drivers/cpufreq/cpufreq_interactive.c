...
#define DEFAULT_SAMPLING_RATE (6 * USEC_PER_MSEC)
#define DEFAULT_ABOVE_HISPEED_DELAY 1100
				       ...
				       unsigned int freq)
{
	...
	spin_lock_irqsave(&tunables->target_load_lock, flags);

	for (i = 0; i < tunables->ntarget_load - 1 &&
	     freq >= tunables->target_load[i + 1]; i += 2)
		;

	ret = tunables->target_load[i];
	spin_unlock_irqrestore(&tunables->target_load_lock, flags);
	return ret;
}
...
static void eval_target_freq(struct interactive_cpu *icpu)
{
	...
	if (cpu_load >= tunables->go_highspeed_load || tunables->boosted) {
		if (policy->cur < tunables->highspeed_freq) {
			new_freq = tunables->highspeed_freq;
		} else {
			new_freq = choose_freq(icpu, loadadjfreq);

			if (new_freq < tunables->highspeed_freq)
				new_freq = tunables->highspeed_freq;
		}
	} else {
		new_freq = choose_freq(icpu, loadadjfreq);
		if (new_freq > tunables->highspeed_freq &&
		    policy->cur < tunables->highspeed_freq)
			new_freq = tunables->highspeed_freq;
	}
	...
	if (!tunables->boosted || new_freq > tunables->highspeed_freq) {
...
static void cpufreq_interactive_boost(struct interactive_tunables *tunables)
{
	...
	for_each_ipolicy(ipolicy) {
		...
		for_each_cpu(i, policy->cpus) {
			...
			if (icpu->target_freq < tunables->highspeed_freq) {
				icpu->target_freq = tunables->highspeed_freq;
...
static ssize_t show_target_load(struct gov_attr_set *attr_set, char *buf)
{
	struct interactive_tunables *tunables = to_tunables(attr_set);
	unsigned long flags;
	ssize_t ret = 0;
	...
	for (i = 0; i < tunables->ntarget_load; i++)
		ret += sprintf(buf + ret, "%u%s", tunables->target_load[i],
			       i & 0x1 ? ":" : " ");
				  ...
				  const char *buf, size_t count)
{
	...
	spin_lock_irqsave(&tunables->target_load_lock, flags);
	if (tunables->target_load != default_target_load)
		kfree(tunables->target_load);
	tunables->target_load = new_target_load;
	tunables->ntarget_load = ntokens;
	spin_unlock_irqrestore(&tunables->target_load_lock, flags);
...
static ssize_t store_highspeed_freq(struct gov_attr_set *attr_set,
				  const char *buf, size_t count)
{
	struct interactive_tunables *tunables = to_tunables(attr_set);
	...
	tunables->highspeed_freq = val;

	return count;
}

static ssize_t store_go_highspeed_load(struct gov_attr_set *attr_set,
				     const char *buf, size_t count)
{
	struct interactive_tunables *tunables = to_tunables(attr_set);
	unsigned long val;
	...
	tunables->go_highspeed_load = val;
...
int cpufreq_interactive_init(struct cpufreq_policy *policy)
{
...
#ifdef CONFIG_KURONEKO_POWERSAVE
	tunables->highspeed_freq = 1339000;
#else
	tunables->highspeed_freq = 1676000;
#endif
	...
	spin_lock_init(&tunables->target_load_lock);
